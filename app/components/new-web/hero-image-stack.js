...
export default function HeroImageStack({ images = [] }) {
  ...
  const getImageStyles = (index) => {
    ...
    if (index === 0) {
      styles += isNearby
        ? " top-0 -left-20 w-[85%] h-[85%]" // More spread out when mouse is nearby
        : " top-0 -left-6 w-[85%] h-[85%]"; // Default position
    } else if (index === 1) {
      styles += isNearby
        ? " top-8 left-4 w-[85%] h-[85%]" // More centered when mouse is nearby
        : " top-8 left-8 w-[85%] h-[85%]"; // Default position
    } else if (index === 2) {
      styles += isNearby
        ? " -top-2 left-28 w-[85%] h-[85%]" // More to the right when mouse is nearby
        : " -top-2 left-20 w-[85%] h-[85%]"; // Default position
    }
  ...
  return (
    <div
      className="relative w-full aspect-[16/10] my-4 lg:my-0"
      ref={containerRef}
    >
